...
// twice.
export function buildCouponCode(workOrder: string, bundleNo: string, opNo: string, cutNo?: string): string {
  ...
  const trimmedBundleNo =
    workOrderDigits && bundleNo.startsWith(workOrderDigits)
      ...
      : bundleNo;
  const cutPart = cutNo ? `-C${cutNo}` : "";
  return `${workOrderDigits || workOrder}${cutPart}-${trimmedBundleNo}-${opNo}`;
}
...
export function demo() {
  const cases: [[string, string, string, string?], string][] = [
    ...
    // cut supplied — inserted as its own segment right after the work order
    [["W/O-003355", "33550001", "OP1", "2"], "3355-C2-0001-OP1"],
    // no cut supplied — same as the no-cut cases, code shape unchanged
    [["W/O-003355", "33550001", "OP1", ""], "3355-0001-OP1"],
  ];
