...
import type { ReportDateRange, ReportSearchMode, ReportSearchSuggestion, ReportSummary } from "../types";
...
export async function fetchEmployeeSearchSuggestions(query: string): Promise<ReportSearchSuggestion[]> {
...
export async function fetchWorkOrderSearchSuggestions(query: string): Promise<ReportSearchSuggestion[]> {
  ...
  const response = await fetch(`/api/open-order/suggestions?${params.toString()}`);
  if (!response.ok) return [];
  const workOrders: string[] = await response.json();
  return workOrders.map((wo) => ({ value: wo, label: wo }));
}

export async function fetchOperationSearchSuggestions(query: string): Promise<ReportSearchSuggestion[]> {
  const params = new URLSearchParams({ query });
  const response = await fetch(`/api/reports/operation-suggestions?${params.toString()}`);
  if (!response.ok) return [];
  const operations: { operationCode: string; operationName: string | null }[] = await response.json();
  return operations.map((op) => ({
    value: op.operationCode,
    label: op.operationName ? `${op.operationCode} — ${op.operationName}` : op.operationCode,
  }));
}
