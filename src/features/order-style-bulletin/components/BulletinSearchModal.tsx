...
interface BulletinSearchModalProps {
  ...
  filteredStyles: StyleBulletinData[];
...
export function BulletinSearchModal({
  findQuery,
  onFindQueryChange,
  filteredStyles,
  allStyles,
              ...
              {filteredStyles.map((style) => (
                ...
                >
                  <td className="px-4 py-3 flex items-center gap-2.5 text-xs font-semibold text-[#334155]">
                    <input
                      type="radio"
                      checked={selectedIndex === allStyles.indexOf(style)}
                      onChange={() => onSelectIndex(allStyles.indexOf(style))}
