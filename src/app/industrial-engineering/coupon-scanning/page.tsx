...
export default function CouponScanningPage() {
  ...
  const executeScanCoupon = async () => {
    ...
    try {
      ...
      setAlreadyDailyScan((prev) => String((parseInt(prev) || 0) + items.length));
      setAlreadyMonthlyScan((prev) => String((parseInt(prev) || 0) + items.length));
  ...
  const handleUnscanCoupon = async (couponCode: string, rowIndex: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setAlreadyDailyScan((prev) => String(Math.max(0, (parseInt(prev) || 0) - 1)));
        setAlreadyMonthlyScan((prev) => String(Math.max(0, (parseInt(prev) || 0) - 1)));
  ...
  const handleSelectWorker = async (worker: any) => {
    ...
    try {
      const response = await fetch(`/api/workers?code=${encodeURIComponent(empCode)}`);
      if (response.ok) {
  ...
  const totalSam = rows.reduce((sum, row) => sum + ((parseFloat(row.smv) || 0) * (parseInt(row.qty) || 0)), 0);
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              ...
              <div className="grid grid-cols-2 gap-2 md:col-span-3">
                <div className="flex flex-col gap-0.5">
                  <span className="font-bold text-[10px] uppercase text-[#4f46e5]">From Cut</span>
                  <input
                    type="text"
                    placeholder="e.g. 1"
                    value={fromCut}
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="font-bold text-[10px] uppercase text-[#4f46e5]">To Cut</span>
                  <input
                    type="text"
                    placeholder="e.g. 10"
                    value={toCut}
          ...
          <table className="w-full border-collapse text-[11px] min-w-[1200px]">
            ...
            <thead>
              ...
              <tr className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                ...
                <th colSpan={3} className="py-0.5 px-3 text-center text-[9px] uppercase tracking-wider bg-slate-200/60 text-slate-800 border-b border-slate-300">
                  Rate
                </th>
              </tr>
              {/* Main table headers */}
              <tr className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200 text-left">
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[40px]">#</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[80px] text-[#ef4444] uppercase tracking-wider text-[9px] font-bold">Unscan</th>
                <th className="py-1 px-3 border-r border-slate-200 w-[140px]">Coupon Code</th>
                <th className="py-1 px-2 border-r border-slate-200 w-[100px]">W/0 #</th>
                <th className="py-1 px-2 border-r border-slate-200 w-[100px]">Cut #</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[80px]">Bundle #</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[70px]">Qty</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[80px]">Inseam</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[70px]">Size #</th>
                <th className="py-1 px-3 border-r border-slate-200 w-[130px]">Section Name</th>
                <th className="py-1 px-3 border-r border-slate-200 w-[180px]">Operation Name</th>
                <th className="py-1 px-2 border-r border-slate-200 text-center w-[60px]">Skill #</th>
                <th className="py-1 px-2 border-r border-slate-200 text-right w-[80px] bg-slate-100/30">SMV</th>
                <th className="py-1 px-2 border-r border-slate-200 text-right w-[90px] bg-slate-100/30">Rate</th>
                <th className="py-1 px-3 text-right w-[100px] bg-indigo-50/20 text-[#4f46e5]">Value</th>
              </tr>
            </thead>
            ...
            <div className="text-xs text-slate-600 mb-4 bg-slate-50 p-3.5 rounded-xl border border-slate-100 flex flex-col gap-2">
              {scanCouponCode.trim() ? (
                ...
                <>
                  ...
                  {fromCut && <div><strong className="text-slate-800">From Cut No:</strong> {fromCut}</div>}
                  {toCut && <div><strong className="text-slate-800">To Cut No:</strong> {toCut}</div>}
